...
{
    A7670C_Result err = A7670C_RequestWithCmd(Test_Handler, &result, os_tick_from_ms(timeout_ms), "AT+CMQTTCFG=?\r\n");
...
{
    A7670C_Result err = A7670C_RequestWithCmd(Read_Handler, result, os_tick_from_ms(timeout_ms), "AT+CMQTTCFG?\r\n");
    return err;
}
...
A7670C_Result A7670C_CMQTTCFG_Write(A7670C_CMQTTCFG_Write_Response* result
        ...
        , uint32_t timeout_ms)
{
    A7670C_Result err = A7670C_RequestWithArgs(Write_Handler, result, os_tick_from_ms(timeout_ms), "AT+CMQTTCFG=\"checkUTF8\",%d,%d\r\n"
            , client_index
            , checkUTF8_flag
            );
    return err;
...
A7670C_Result A7670C_CMQTTCFG_Write2(A7670C_CMQTTCFG_Write_Response* result
        ...
        , uint32_t timeout_ms)
{
    A7670C_Result err = A7670C_RequestWithArgs(Write_Handler, result, os_tick_from_ms(timeout_ms), "AT+CMQTTCFG=\"optimeout\",%d,%d\r\n"
            , client_index
            , optimeout_val
    );
    return err;
