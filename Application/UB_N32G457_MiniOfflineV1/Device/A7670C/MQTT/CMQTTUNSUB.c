...
{
    A7670C_Result err = A7670C_RequestWithCmd(Test_Handler, &result, os_tick_from_ms(timeout_ms), "AT+CMQTTUNSUB=?\r\n");
        ...
        , uint32_t timeout_ms)
{
    ...
    A7670C_Result err;
    if(dup==kA7670C_Bool_Unspecified){
        err = A7670C_RequestWithArgs(Write2_Handler, &request, os_tick_from_ms(timeout_ms)
                , "AT+CMQTTUNSUB=%d,%d,%d\r\n"
                , client_index
                , topic_length
                , kA7670C_Bool_No
