...
{
    A7670C_Result err = A7670C_RequestWithCmd(Test_Handler, &result, os_tick_from_ms(timeout_ms), "AT+CMQTTDISC=?\r\n");
...
{
    A7670C_Result err = A7670C_RequestWithCmd(Read_Handler, result, os_tick_from_ms(timeout_ms), "AT+CMQTTDISC?\r\n");
    return err;
}
...
A7670C_Result A7670C_CMQTTDISC_Write(A7670C_CMQTTDISC_Write_Response* result
        ...
        , uint32_t timeout_ms)
{
    A7670C_Result err = A7670C_RequestWithArgs(Write_Handler, result, os_tick_from_ms(timeout_ms), "AT+CMQTTDISC=%d,%d\r\n"
            , client_index
            , timeout);
    return err;
}
