...
{
    A7670C_Result err = A7670C_RequestWithCmd(Test_Handler, &result, os_tick_from_ms(timeout_ms), "AT+CMQTTWILLTOPIC=?\r\n");
        ...
        , uint32_t timeout_ms)
{
    ...
    A7670C_Result err = A7670C_RequestWithArgs(Write_Handler, &request, os_tick_from_ms(timeout_ms), "AT+CMQTTWILLTOPIC=%d,%d\r\n"
            , client_index
            , topic_length);
    return err;
}
