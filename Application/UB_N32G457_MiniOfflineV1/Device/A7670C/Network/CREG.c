...
{
    A7670C_CREG_Read_Response* result = (A7670C_CREG_Read_Response*)ud;
    sdk_ringbuffer_text_t find_result;

    if(sdk_ringbuffer_find_str(buffer, 0, "OK\r\n")!=-1 /*接收结束*/){
        result->code=kA7670C_Response_Code_OK;
        int find = sdk_ringbuffer_cut(&find_result, buffer, 0, sdk_ringbuffer_used(buffer),"+CREG: ", "\r\n");
        if(find==0){
            int pEnd=0;
...
{
    result->err_code = 0;
    A7670C_Result err = A7670C_RequestWithCmd(Read_Handler, result, os_tick_from_millisecond(timeout_ms), "AT+CREG?\r\n");
    return err;
}
...
{
    A7670C_CREG_Write_Response* result = (A7670C_CREG_Write_Response*)ud;
