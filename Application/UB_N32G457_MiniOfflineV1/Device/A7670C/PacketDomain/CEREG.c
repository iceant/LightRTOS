...
{
    A7670C_Result err = A7670C_RequestWithCmd(Test_Handler, &result, os_tick_from_ms(timeout_ms), "AT+CEREG=?\r\n");
...
{
    A7670C_Result err = A7670C_RequestWithCmd(Read_Handler, result, os_tick_from_ms(timeout_ms), "AT+CGREG?\r\n");
    return err;
}
...
A7670C_Result A7670C_CEREG_Write(A7670C_CEREG_Write_Response* result, int n, uint32_t timeout_ms)
{
    A7670C_Result err;
    result->err_code = -1;
    err = A7670C_RequestWithArgs(Write_Handler, result, os_tick_from_ms(timeout_ms), "AT+CEREG=%d\r\n", n);
    return err;
}
...
{
    A7670C_Result err;
    err = A7670C_RequestWithArgs(Exec_Handler, result, os_tick_from_ms(timeout_ms), "AT+CEREG\r\n");
    return err;
}